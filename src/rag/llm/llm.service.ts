  ...
  private initializeModel(): void {
    try {
      this.chatModel = new ChatOpenAI({
        openAIApiKey: process.env.API_KEY || 'ollama', // Ollama 通常不需要真实的 API key
        modelName: process.env.LLM_MODEL || 'llama2',
        temperature: 0.1,
        maxTokens: 2000,
        configuration: {
      ...
      this.logger.log(`LLM service initialized with model: ${process.env.LLM_MODEL}`);
      this.logger.log(`Base URL: ${process.env.BASE_URL}`);
    } catch (error) {
